 ...
 *  5. Prototype-pollution guard   — strips __proto__ / constructor from req.body
 *  6. Parameter sanitisation      — trims & truncates string query params before
...
// ---------------------------------------------------------------------------
// 3. Global rate limiter — 200 req / 15 min per IP (generous for a dev API)
//    Individual sensitive routes (auth) use tighter limits; see auth.routes.js.
// ---------------------------------------------------------------------------
...
//    Strips dangerous keys from req.body so that a crafted payload like
//    { "__proto__": { "admin": true } } can never pollute Object.prototype.
//    Zod schemas already reject unexpected keys, but this is defence-in-depth.
// ---------------------------------------------------------------------------
...
function stripDangerousKeys(obj) {
  ...
  for (const [key, value] of Object.entries(obj)) {
    if (DANGEROUS_KEYS.has(key)) continue; // drop it silently
    clean[key] = stripDangerousKeys(value);
  }
  return clean;
}

export function protoGuard(req, res, next) {
...
// ---------------------------------------------------------------------------
// 5. Query parameter sanitisation
